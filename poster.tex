...
\headerbox{lightsheet microscopy}{name=lightsheet, span=3, column=0, row=0}{
  ...
  \begin{minipage}[t]{\textwidth}
    ...
    Consider the problem of localising $k$ non-negative point sources on the
    interval $[0,1]$, namely finding their 
    locations $ t_1, \ldots, t_k $ and magnitudes $ a_1, \ldots, a_k \geq 0$
    from $m$ noisy samples $ y_1, \ldots, y_m $ which 
    consist of the convolution of the input signal with a 
    known kernel $\phi$ (e.g. Gaussian $\phi(t) = e^{-t^2/\sigma^2}$) and 
    additive noise $\mathbf{w}$ bounded by $\delta$. 
  \end{minipage}
  \vspace{1pt}
    
  \begin{minipage}[t]{\textwidth}
  %\begin{tcbitemize}[raster columns=2, raster equal height]
    %\tcbitem[title={Input signal}]
    \begin{minipage}[t]{0.49\textwidth}
      %\subsection*{\textsc{Input signal}}
      \begin{center}
        \larger
        {\color{blue}\textbf{\textsc{Input signal}}}\\
    ...
    \begin{minipage}[t]{0.49\textwidth}
      ...
      \begin{center}
        \tcbhighmath[colback=red!10!white, frame hidden]{
          y_j = \int_{[0,1]} \phi(t-s_j) x(\dif t) + w_j, 
          %\quad \|w\|_2 \le \delta
        }
      \end{center}
